            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between py-3">
                        ...
                        <a href="{{ route('booking-offices.create') }}">
                    ...
                    <div class="table-responsive">
                        <table id="dataTableExample" class="table">
                            ...
                            <tbody>
                                @foreach ($booking_offices as $booking_office)
                                    <tr>
                                        <td>{{ $booking_office->id }}</td>
                                        <td>{{ $booking_office->company_name }}</td>
                                        <td>{{ $booking_office->booking_office_name }}</td>
                                        <td>{{ $booking_office->booking_office_address }}</td>
                                        <td>{{ $booking_office->booking_office_number }}</td>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu border rounded"
                                                    ...
                                                    <a class="dropdown-item d-flex align-items-center p-2"
                                                        href="{{ route('booking-offices.edit', $booking_office->id) }}"><i
                                                            data-feather="edit-2" class="icon-md mr-2"></i> <span
                                                            class="">Edit</span></a>
                                                    <form action="{{ route('booking-offices.destroy', $booking_office->id) }}"
                                                        id="delete_item_from{{ $booking_office->id }}" method="post">
                                                        @csrf
                                                        @method('DELETE')
                                                        <a type="button" class="dropdown-item d-flex align-items-center p-2"
                                                            title="Delete"
                                                            onclick="deleteConfirmation('delete_item_from{{ $booking_office->id }}')"><i
